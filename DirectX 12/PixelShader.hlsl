...
float4 main(OUTPUT_TO_RASTERIZER inputPixel) : SV_TARGET
{
    ...
    float4 color = float4(instanceInfo.KdX, instanceInfo.KdY, instanceInfo.KdZ, 1.0f) * cameraAndLights.sunColor * clamp(saturate(dot(-cameraAndLights.sunDir, float4(inputPixel.nrmW, 1.0f))) + cameraAndLights.sunAmbient, 0.0f, 1.0f);
